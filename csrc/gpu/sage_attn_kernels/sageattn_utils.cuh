...
#define DISPATCH_A8W8(a8w8, A8W8, ...)                            \
  if (a8w8) {                                                     \
    constexpr bool A8W8 = true;                                   \
    __VA_ARGS__                                                   \
  } else {                                                        \
    constexpr bool A8W8 = false;                                  \
    __VA_ARGS__                                                   \
  }
...
template <uint32_t num_frags_x,
          ...
          uint32_t stride=8>
__device__ __forceinline__ void write_o_reg_gmem_shift_smooth_quant_sageattn(
...
CUtensorMap create_tensor_map_3D(T* gmem_ptr, int d1, int d2, int d3, int stride1, int stride2) {
    ...
    uint64_t gmem_prob_shape[5] = {(uint64_t)d3, (uint64_t)d2, (uint64_t)d1, 1, 1};  // 3D -> 5D 填充
    uint64_t gmem_prob_stride[5] = {(uint64_t)stride2 * sizeof(T), (uint64_t)stride1 * sizeof(T), 0, 0, 0};
    uint32_t smem_box_shape[5] = {uint32_t(BlockMinorSize), uint32_t(BlockMajorSize), 1, 1, 1};
    uint32_t smem_box_stride[5] = {1, 1, 1, 1, 1};

    CUresult result = cuTensorMapEncodeTiled(
        &tma_map, (sizeof(T) == 2) ? CU_TENSOR_MAP_DATA_TYPE_BFLOAT16 : CU_TENSOR_MAP_DATA_TYPE_UINT8, 
        3,  // 3D 张量
...
CUtensorMap create_tensor_map_4D(T* gmem_ptr, int d1, int d2, int d3, int d4, int stride1, int stride2, int stride3) {
    ...
    CUresult result = cuTensorMapEncodeTiled(
        &tma_map, (sizeof(T) == 2) ? CU_TENSOR_MAP_DATA_TYPE_BFLOAT16 : CU_TENSOR_MAP_DATA_TYPE_UINT8, 
        4,  // tensor rank, the dim of tensor array, must be 3, 4, 5
        gmem_address, 
        gmem_prob_shape,
...
template <typename T>
__device__ __forceinline__ void load_async_3D(T *dst, void const* const src_tma_map, 
                                            uint64_t* bar, int s0, int s1, int s2) {
